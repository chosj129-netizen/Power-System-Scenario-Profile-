...
def add_soc_simulation_columns(
    ...
    for _, row in df.iterrows():
        ...
        # Positive ess_energy_kwh means discharge (BTO): battery supplies energy
        if ess_energy_kwh > 0:
            battery_energy_end = battery_energy_start - ess_energy_kwh
        elif ess_energy_kwh < 0:
            # negative means net charge (BTI)
...
def render_voyage_scenario_planner(
    ...
    if st.session_state.get("show_generator_sizing", False):
        ...
        if generator_only_applied:
            ...
            # add ESS generator and battery capacity baselines for capacity series (use requested internal names)
            load_profile_df["ess_gen_capacity_kw"] = round(float(ess_gen_kw), 1)
            load_profile_df["battery_capacity_kwh"] = round(float(batt_kwh), 1)

            # ensure numeric and round to 1 decimal
            for c in ["total_load_kw", "ess_gen_capacity_kw", "battery_capacity_kwh", "ess_energy_kwh"]:
            ...
            legend_labels = {
                ...
                "ess_gen_capacity_kw": "ess_gen_capacity (kW)",
                "battery_capacity_kwh": "battery_capacity (kWh)",
                "ess_energy_kwh": "ess_energy (kWh)",
            }
            chart_df["series_label"] = chart_df["series"].map(legend_labels).fillna(chart_df["series"])

            # color and dash settings (single left y-axis only)
            domain = list(legend_labels.values())
            color_range = ["#1f77b4", "#ff7f0e", "#9467bd", "#2CA02C"]
            dash_map = [[], [6, 3], [2, 2], [4, 2]]
            ...
            # area for ess_energy_kwh
            area = (
                alt.Chart(chart_df)
                .transform_filter(alt.datum.series == "ess_energy_kwh")
                .mark_area(opacity=0.25, color="#2CA02C")
                .encode(
                    x=base_x,
            ...
            chart = alt.layer(area, lines).properties(width="container", height=360)
